...
export default function Learn() {
	...
	useEffect(() => { document.body.style.overflow = showPopup ? 'hidden' : 'unset'; }, [showPopup]);
	...
	const tipsAndTutorials = [
		...
		{ label: "When is the best time to invest in crypto?", description: "Learn how to setup and get started with a crypto wallet.", image: "https://images.ctfassets.net/q5ulk4bp65r7/3K4qo02ZA5PkwyN5Rm7gjm/945bce812fc91da9ef737516142eb281/Dollar-Cost_avg.png?w=768&fm=png", href: "#" },
		...
		<div className="min-h-screen flex flex-col bg-white">
			...
			<main className="flex-1 mt-[72px]">
				<section className="py-16 md:py-24 px-6 w-full max-w-[1200px] mx-auto">
					<h1 className="text-4xl md:text-[56px] leading-[1.1] font-display font-medium text-black mb-6">Crypto questions, answered</h1>
					<p className="text-gray-60 text-xl font-body max-w-2xl">Beginner guides, practical tips, and market updates for first-timers, experienced investors, and everyone in between</p>
				</section>
						...
						<div className="lg:col-span-2">
							...
							<a href="#" className="group block relative rounded-2xl overflow-hidden bg-gray-5 border border-transparent hover:border-gray-20 transition-all shadow-sm hover:shadow-md">
								<div className="aspect-video w-full relative overflow-hidden">
									<img src="https://images.ctfassets.net/q5ulk4bp65r7/3K4qo02ZA5PkwyN5Rm7gjm/945bce812fc91da9ef737516142eb281/Dollar-Cost_avg.png?w=768&fm=png" alt="Dollar cost averaging" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"/>
								...
								<div className="p-6 md:p-8 bg-white transition-colors">
									<span className="text-sm font-semibold text-gray-60 tracking-wider uppercase mb-3 block">Video Tutorial</span>
									<h3 className="text-3xl font-display font-medium text-black mb-4 group-hover:text-blue-60 transition-colors">When is the best time to invest in crypto?</h3>
									<p className="text-lg text-gray-60">When prices are fluctuating, how do you know when to buy? Learn more about using dollar-cost averaging to weather price volatility.</p>
								</div>
							</a>
						</div>
						<div>
				...
				<section className="py-16 px-6 w-full max-w-[1200px] mx-auto border-t border-gray-15">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						{cryptoBasics.map((item, i) => (<div key={i} className="flex h-full"><LearnCard {...item} /></div>))}
					</div>
				</section>

				<section className="py-24 px-6 w-full mx-auto bg-gray-5 border-t border-gray-15">
					<div className="max-w-[800px] mx-auto">
						<div className="mb-12 text-center"><h2 className="text-[40px] font-display font-medium text-black">What is...</h2></div>
						<div className="flex flex-wrap justify-center gap-4 mb-12">
							{whatIs.map((item, i) => (<a key={i} href="#" className="px-5 py-3 bg-white rounded-lg shadow-sm border border-transparent hover:border-gray-20 transition-all text-[15px] font-medium text-black hover:text-blue-60">{item.label}</a>))}
						</div>
						<div className="flex justify-center"><a href="#" className="px-6 py-3 bg-blue-60 text-white font-medium rounded-lg hover:bg-blue-70 transition-colors">See more</a></div>
					</div>
				</section>
				...
				<section className="py-16 px-6 w-full max-w-[1200px] mx-auto border-t border-gray-15">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
						{tipsAndTutorials.map((item, i) => (<div key={i} className="flex h-full"><LearnCard {...item} /></div>))}
					</div>
				</section>
				...
				<section className="py-16 px-6 w-full max-w-[1200px] mx:auto border-t border-gray-15">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
						{advancedTrading.map((item, i) => (<div key={i} className="flex h-full"><LearnCard {...item} /></div>))}
					</div>
				</section>
				...
				<section className="py-16 px-6 w-full max-w-[1200px] mx:auto border-t border-gray-15">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
						{allThingsWallet.slice(0, 4).map((item, i) => (<div key={i} className="flex h-full"><LearnCard {...item} /></div>))}
					</div>
				</section>
			</main>
			<Footer />
